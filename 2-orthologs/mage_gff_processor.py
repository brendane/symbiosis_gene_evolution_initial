#!/usr/bin/env python3
"""
    Extract certain pieces of information from a GFF3 file created by MaGe.

    mage_gff_processor.py <gff3 file> <prefix>
"""
...
sys.stdout.write('locus_tag\tname\tproduct\tpartial\tpseudo\n')
for tags in all_tags.values():
    lt = tags['locus_tag']
    gene = tags['gene']
    product = upr.unquote(tags['product'])
    partial = False; pseudo = False
    if product.endswith('(fragment)') or product.startswith('fragment of'):
        partial = True
    if tags['pseudo'] == 'None':
        pseudo = True
